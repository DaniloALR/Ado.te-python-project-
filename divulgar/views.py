...
def novo_pet(request):
    if request.method == 'GET':
        ...
        if len(nome.strip()) == 0 or len(descricao.strip()) == 0 or len(estado.strip()) == 0 or len(cidade.strip()) == 0 or len(telefone.strip()) == 0 or len(tags) == 0:
            messages.add_message(request, constants.ERROR, 'Preencha todos os campo vazios')
            tags = Tag.objects.all()
            racas = Raca.objects.all()
            return render(request, 'novo_pet.html', {'tags': tags, 'racas': racas})
